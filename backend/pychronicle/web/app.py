...
This is now a pure API — no static files are served here. The
dashboard lives separately under /frontend (open frontend/index.html,
or serve that folder with any static file server). It computes its
own dashboard/snapshot views client-side from /api/timeline +
/api/variables, so those two + /api/sessions + /api/parse are the
routes it actually depends on. CORS is left wide open (allow_origins
"*") so the frontend can be served from any origin/port.
"""
